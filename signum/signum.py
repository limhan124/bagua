...
class SignumAlgorithmImpl(AlgorithmImpl):
    def __init__(
        ...
        self.send_tensor = torch.zeros(590722, device='cuda', dtype=torch.uint8)
        ...
        self.sign_momentum = torch.zeros(4725770, device=self.send_tensor.device, dtype=torch.float32)
    ...
    def init_operations(self, bagua_ddp: BaguaDistributedDataParallel, bucket: BaguaBucket, ):
        ...
        def compress_momentum_majority_vote(*args):
            ...
            param_num = self.compressor.element_num(send_tensor.size())
            self.send_tensor[:param_num].copy_(send_tensor)
            ...
            for tensor, shape in zip(bucket.tensors, shapes):
                ...
                tensor.grad.copy_(self.sign_momentum[cur_idx:cur_idx + num].view(shape))
                cur_idx += num

            # print("bucket_size:", cur_idx)
...
# from bagua.torch_api.data_parallel.bagua_distributed import BaguaDistributedDataParallel
# from bagua.torch_api.algorithms import Algorithm, AlgorithmImpl
# from bagua.torch_api.communication import BaguaProcessGroup, allgather, allreduce
# from torch.optim.optimizer import Optimizer
# import torch
# import math
# from typing import List
...
#                 #     device=self.send_tensor.device, dtype=self.send_tensor.dtype
#                 # )
#                 _minmax, self.send_tensor = self.compressor.compress(send_tensor_list)
#                 self.recv_tensor_list = torch.zeros(self.send_tensor, device=self.send_tensor.device,
#                                                     dtype=self.send_tensor.dtype)
#             else:
#                 send_tensor_list = torch.cat(send_tensor_list)
#                 _minmax, send_tensor = self.compressor.compress(send_tensor_list)
#                 # send_tensor, size = self.compressor.packing(send_tensor_list)
#                 self.send_tensor.copy_(send_tensor)
#             # # allgather of the whole bucket
#             # allgather(self.send_tensor, self.recv_tensor_list)
...
#             allreduce(self.send_tensor, self.recv_tensor_list)
#             self.sign_momentum = self.compressor.decompress(_minmax, self.recv_tensor_list)
#             # change tensor.grad
#             cur_idx = 0
#             for tensor, shape in zip(bucket.tensors, shapes):
#                 num = self.compressor.element_num(shape)
